...
public class Enemy : MonoBehaviour
{
    ...
    public void TakeDamage(int dmg)
    {
        ...
        else if (hp <= 0)
        {
            ...
            if (eName == "5")
            {
                GameManager.instance.soundManager.PlaySFX("Explosion_Boss");
            }
            else
            {
                GameManager.instance.soundManager.PlaySFX("Explosion");
            }
    ...
    void Fire()
    {
...

        {
            if (eName == "1")
            {
                ...
                Vector3 dirVecR = Vector3.down;
                Vector3 dirVecL = Vector3.down;
    ...
    void FireShot()
    {
        ...
        Vector2 dirVec = playerTransform.position - transform.position; // 초기화 추가
    ...
    public void DestroyEndAnimation()
    {
        ...
        speed = data.speed;
        spawnType = data.spawnType;
        Debug.Log("체력이 업데이트 됨");

        if (eName == "4")
        {
            GameManager.instance.objectManager.DeleteAllObj(eName);
        }
