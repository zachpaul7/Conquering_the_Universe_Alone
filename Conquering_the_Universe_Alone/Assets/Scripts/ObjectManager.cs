...
public class ObjectManager : MonoBehaviour
{
    ...
    private void Generater()
    {
        ...
        {
            bullet_Player_Cannon[index] = Instantiate(bullet_Player_Cannon_Prefab);
            bullet_Player_Cannon[index].SetActive(false);
        }

        for (int index = 0; index < bullet_Player_Rocket.Length; index++)
        {
            bullet_Player_Rocket[index] = Instantiate(bullet_Player_Rocket_Prefab);
            bullet_Player_Rocket[index].SetActive(false);
        }

        for (int index = 0; index < bullet_Player_BFGun.Length; index++)
        {
            bullet_Player_BFGun[index] = Instantiate(bullet_Player_BFGun_Prefab);
